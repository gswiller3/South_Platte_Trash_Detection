...
class Config:
    ...
    # Classes to detect (COCO classes that could be trash in river)
    # We'll use a general model first, then filter for trash-like objects
    TRASH_CLASSES = {
    ...
    # Processing settings
    BATCH_SIZE = 8  # Images to process at once (adjust based on GPU memory)
    IMAGE_SIZE = 1280  # Input size for model (higher = better small object detection)
    ...
    def _process_results(self, results, image_path: str) -> Dict:
        ...
        if results.boxes is not None:
            ...
            for box, conf, cls_id in zip(boxes, confidences, class_ids):
                ...
                # For general COCO model, we consider all detections as potential trash
                # In a river environment, most objects are trash
                detections.append({
                    'class': class_name,
